...
import { GetSong, GetArtist, GetArtistinfo, GetAlbuminfo, GetMusicinfo } from './service component/server_request_functions';
    ...
    useEffect(() => {
        ...
        if (!load_value) {
            try {
                ...
                GetArtist().then((data) => getArtistData(data)).catch((error) => {
                    if (error.message == 'Network Error') {
                        let w = document.getElementsByClassName('main');
                        w[0].style.display = 'none';
                        let k = document.getElementsByClassName('error_div');
            ...
            <div className='main'>
                <div className='album'>
                    ...
                    <div className="albumflex">
                        <div className="topbox" onMouseEnter={getelement_top} onMouseLeave={removeelement_top} id='liked songs' onClick={()=>{window.alert('You cannot access liked songs without User authentication, but you can enjoy music')}}>
                            <i className="bi bi-heart-fill" id='likedalbum' />
                            <div className="topbox_text">
                                Liked Songs
                            </div>
                            <img src={p} className='playicon_design' onClick={playaudio} id='liked songs play' />

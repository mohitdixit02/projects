...
import { GetMusicinfo, GetArtistinfo, GetAlbuminfo } from './service component/server_request_functions';
            ...
            <div className="catgflex">
              {
                genere.map((value, index) => {
                  return (
                    <div className="catg" key={index} style={{ 'backgroundColor': `${backgroundColor_generator()}` }}>
                      ...
                      song.map((song, index) => {
                        ...
                        return (
                          <tr id={song.id} key={index} className='songlist_search_active' onClick={playsongtd}>
                            ...
                            <td className='songtd' style={{ 'borderBottomRightRadius': '5px', 'borderTopRightRadius': '5px', 'width': '20%' }}><div style={{ 'display': 'flex', 'columnGap': '30px', 'paddingRight': '5px' }}><i className="bi bi-heart"></i><span>{duration}</span></div></td>
                          </tr>
                        )
                      })
                    }
